...
if (typeof cpb_slider == 'undefined' ) {
	var cpb_slider = function( show_elem ) {
		this.show = show_elem;
		this.type = show_elem.data('type');
		this.auto = show_elem.data('autorotate');
		this.transition = show_elem.data('transition');
		this.change_mobile = show_elem.data('changem');
		this.mobile_break = show_elem.data('breakpoint');
		this.delay = show_elem.data('delay');
		this.slides = show_elem.find('.slide');
		var self = this;
		this.init = function() {
			...
			jQuery( window ).resize( function(){self.do_change_mobile() } );
			self.add_secondary_nav();
		}
		this.do_change_mobile = function(){
			if ( jQuery('body').width() < self.mobile_break ) {
				switch( self.change_mobile ) {
					case 'gallery-slideshow':
						self.show.removeClass('vertical').addClass('gallery-slideshow');
						self.slides.removeAttr('style');
						self.transition = 'slide';
						break;
				}
			} else if ( self.transition !== self.show.data('transition') ) {
				switch( self.show.data('transition') ){
					case 'slide-vertical':
						self.slides.removeAttr('style').removeClass('active-slide');
						self.show.removeClass('gallery-slideshow').addClass('vertical');
						var after_slide = self.show.find('.slide.after-slide');
						var active_slide = self.get_next_slide( after_slide, -1 );
						active_slide.addClass('active-slide');
						self.transition = 'slide-vertical';
						break;
				}
			}
		}
		...
		this.get_next_slide = function( slide, dir) {
			if ( dir > 0 ) {
				var n_slide = slide.next('.slide');
				if ( n_slide.length < 1 ) {
					n_slide = self.show.find('.slide').first();
				}
			} else {
				var n_slide = slide.prev('.slide');
				if ( n_slide.length < 1 ) {
					n_slide = self.show.find('.slide').last();
				}
			}
			return n_slide;
		}
		...
		this.slide_vertical = function(dir,n,c) {
			var s_wrap = c.closest('.slides');
			var s_before = this.show.find('.before-slide');
			var s_before_before = self.get_next_slide( s_before, -1 );
			var s_after = this.show.find('.after-slide');
			var s_after_after = self.get_next_slide( s_after, 1 );
			var s_current = c;
			var s_height = c.outerHeight();
			s_wrap.css('height', s_height );

			if ( dir > 0 && 0 < s_after.length ) {
				s_after_after.css('top','200%');
				s_after_after.animate( {top:'100%','opacity':0.25}, 300 );
				s_before.animate( {top:'-200%','opacity':0 }, 300 );
				s_current.animate( {top:'-100%','opacity':0.25}, 300, function(){
					s_before.removeClass('before-slide');
					s_current.removeClass('active-slide').addClass('before-slide');
					s_after.addClass('active-slide').removeClass('after-slide');
					s_after_after.addClass('after-slide');
				} );
				s_after.animate( {top:'0','opacity':1}, 300 );
			} else if ( 0 < s_before.length ) {
				s_before.animate( {top:'0','opacity':1 }, 300 );
				s_current.animate( {top:'100%','opacity':0.25}, 300, function(){
					s_before.removeClass('before-slide').addClass('active-slide');
					s_current.removeClass('active-slide').addClass('after-slide');
					s_after.removeClass('after-slide');
					s_before_before.addClass('before-slide');
				} );
				s_after_after.css('top','-200%');
				s_before_before.animate( {top:'-100%','opacity':0.25}, 300 );
				s_after.animate( {top:'200%','opacity':0}, 300 );
			}
			...
			n.css( {top: s_top})
			n.animate( {top:'0'}, 750 );
			c.animate( {top:c_top}, 750, function() {
				self.reset_show(n,c);
			} );*/
		}
		this.reset_show = function(n,c){
